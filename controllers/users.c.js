...
// POST
    router.post('/', function (req, res) {
        Model.find({userEMail: req.body.userEMail}, function (err, data) {
            if (err) return res.status(500).send("There was a problem finding the data.");
            if (data.length>=1)
                {
                 return res.status(409).send("User created before.");
                } 
                else
                { 
                    var pack = new Model();
...
// DELETE
    router.delete('/:id', function (req, res) {
        Model.findByIdAndRemove(req.params.id, function (err, data) {
            ...
            res.status(200).send("User : "+ req.params.userEMail +" was deleted.");
        });
    });
